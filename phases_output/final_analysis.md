# Final Analysis (Config: GEMINI_FLASH)
...
# Your Requirements:
1.  Your primary goal is to generate an `AGENTS.md` file that **STRICTLY ADHERES** to the ARS-1 specification.
2.  The `AGENTS.md` file must be highly detailed, providing comprehensive context for AI agents working on the `agentrules-architect` codebase.
3.  **Prioritize** addressing the identified weaknesses, especially token limit management and LLM output parsing fragility, by incorporating strategies into the `AGENTS.md`.
4.  ONLY USE Python 3.11+ code examples when necessary.
5.  Maintain a professional, clear, and concise tone throughout the `AGENTS.md` content.
6.  Emphasize "new" and "advanced" concepts where applicable to the 2025 temporal framework.
7.  Ensure all generated content is directly relevant to the `agentrules-architect` project's structure, technologies, and challenges.
8.  **NEVER** include the project report or project structure directly in the `AGENTS.md` output.
