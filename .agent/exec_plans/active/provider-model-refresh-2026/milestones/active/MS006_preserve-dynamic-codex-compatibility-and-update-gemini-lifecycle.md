...
created: 2026-07-15
updated: 2026-07-15
tags: [codex, gemini, runtime-catalog, lifecycle]
risk: med
links:
  issue: ""
...
- [ ] Live Codex catalog efforts `max` and `ultra` survive normalization, appear in dynamic preset choices, serialize into preset keys, and reach turn requests unchanged.
- [ ] A future unknown but syntactically safe catalog effort is retained after known ordered values; malformed or untrusted values are rejected.
- [ ] Codex's runtime-default sentinel continues to follow the model marked default by the installed runtime/account.
- [ ] No static Codex GPT-5.6 Sol/Terra/Luna entries are added to `BASE_MODEL_PRESETS`.
- [ ] Gemini 2.5 Flash and Pro labels/descriptions disclose their 2026-10-16 shutdown and recommend Gemini 3.5 Flash or 3.1 Pro as appropriate.
- [ ] Saved keys for already retired/redirected Gemini preview endpoints continue to load but send their canonical active replacement model, with labels disclosing the redirect.
- [ ] Stable `gemini-3.5-flash` remains unchanged and continues to use medium thinking and its existing capability profile.
- [ ] Focused Codex catalog/config/CLI, Gemini capability, model-override, import, Ruff, and Pyright validation passes.
...
| Codex capability discovery | Partial | New effort labels are silently normalized to `None`. |
| Gemini current model | Healthy | Stable Gemini 3.5 Flash already exists and passes capability tests. |
| Gemini lifecycle UX | Stale | Active picker retains dead previews and unmarked upcoming 2.5 shutdowns. |
...
| --- | --- | --- | --- |
| A1 | Types | Carry validated runtime effort strings without a stale closed literal. | Planned |
| A2 | Normalization | Preserve known and future safe tokens; reject malformed values. | Planned |
| A3 | Ordering/labels | Order known values and format unknown values deterministically. | Planned |
| A4 | Request | Prove dynamic preset effort reaches `turn/start`. | Planned |

### Workstream B - Gemini lifecycle

| ID | Area | Description | Status |
| --- | --- | --- | --- |
| B1 | Deprecation | Label 2.5 shutdown dates and replacements. | Planned |
| B2 | Redirects | Map already dead preview keys to active canonical IDs. | Planned |
| B3 | Stability | Keep 3.5 Flash config/capability behavior unchanged. | Planned |
| B4 | UX tests | Verify picker labels, keys, and model-name mappings. | Planned |

### Workstream C - Evidence and docs

| ID | Area | Description | Status |
| --- | --- | --- | --- |
| C1 | Fixtures | Add max/ultra/future/malformed catalog cases. | Planned |
| C2 | Unit tests | Cover parse, build, serialization, and request propagation. | Planned |
| C3 | Docs | Update live-catalog and Gemini lifecycle guidance. | Planned |
