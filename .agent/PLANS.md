...
When the ExecPlan is complete, move the full directory to `.agent/exec_plans/complete/YYYY/MM/DD/EP-YYYYMMDD-NNN_<short-slug>/` with `agentrules execplan complete EP-YYYYMMDD-NNN` (optionally `--date YYYYMMDD`). Legacy `.agent/exec_plans/completed/...`, `.agent/exec_plans/archive/...`, and `agentrules execplan archive ...` remain supported.
